		...
		func() error {
			...
			if len(it.lastScrollID) <= 0 {
				...
				resp, it.err = it.client.Scroll(it.indices...).KeepAlive(keepalive).
					...
					SearchSource(it.searchSource).Size(it.pageSize).Sort("start_time", ascending).Do(ctx)
				if it.err != nil {
					return it.err
				}
			} else {
...
func parseHits(hits []*elastic.SearchHit) (list []*trace.Span) {
...
func parseData(bytes []byte) (*trace.Span, error) {
	var data trace.Span
