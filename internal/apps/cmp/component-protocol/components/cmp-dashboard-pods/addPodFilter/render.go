...
func init() {
	base.InitProviderWithCreator("cmp-dashboard-pods", "addPodFilter", func() servicehub.Provider {
		return &ComponentAddPodFilter{}
	})
}

var steveServer cmp.SteveServer

func (f *ComponentAddPodFilter) Init(ctx servicehub.Context) error {
...
func (f *ComponentAddPodFilter) Render(ctx context.Context, component *cptype.Component, _ cptype.Scenario,
	event cptype.ComponentEvent, gs *cptype.GlobalStateData) error {
	f.InitComponent(ctx)
	if err := f.GenComponentState(component); err != nil {
		return fmt.Errorf("failed to gen addPodFilter component state, %v", err)
	}
	...
	if err := f.SetComponentValue(ctx); err != nil {
		return fmt.Errorf("failed to set addPodFilter component value, %v", err)
	}
